...
{
    mbed_error_printf("\nState: 0x%x Entry: 0x%x Stack Size: 0x%x Mem: 0x%x SP: 0x%x", thread->state, thread->thread_addr, thread->stack_size, (uint32_t)thread->stack_mem, thread->sp);
}
...
MBED_NOINLINE void print_context_info(void)
{
    //Context Regs
    for(int i=0;i<13;i++) {
        mbed_error_printf("\nR%d   : %08X", i, ((uint32_t *)&mbed_fault_context)[i]);  
    }
    ...
    if(((0xFFFF0000 & SCB->CFSR) >> 16) & 0x80) {
        mbed_error_printf("\nBFA : %08X",SCB->BFAR); 
    }
#endif
